...
export default {
  ...
  methods: {
    ...
    getInitData(innerCode) {
      ...
      let that = this;
      this.$axios
        .post(this.$api.GetShFinalRstDetailByInnerCode, {
          innerCode
        })
        .then(function(response) {
          if (response.data.status == 1) {
            ...
            that.$message.error(
              `GetShFinalRstDetailByInnerCode错误：${response.data.message}`
            );
          }
        })
        .catch(function(error) {
    ...
    saveChange() {
      ...
      //发送请求
      this.$axios
        .post(this.$api.UpdateShFinalRstDetail, that.entity)
        .then(function(response) {
          if (response.data.status == 1) {
            ...
            that.$message.error(
              `UpdateShFinalRstDetail200错误：${response.data.message}`
            );
          }
        })
        .catch(function(error) {
          that.$message.error(`UpdateShFinalRstDetail错误：${error}`);
        });
    },
    getinit() {
      let that = this;
