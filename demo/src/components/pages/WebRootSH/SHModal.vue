<template>
  <div class="content">
    ...
    <shdialogmodal ref="shdialogmodal" :name="shdialogmodalName"></shdialogmodal>
    <shdialogaddabnormal ref="shdialogaddabnormal"></shdialogaddabnormal>
    <shdialogcompare ref="shdialogcompare" :operatorCod="operatorCode" :orderCod="orderCode"></shdialogcompare>
    <shdialogaddadviece ref="shdialogaddadviece"></shdialogaddadviece>
    <shdialogeditadviece ref="shdialogeditadviece"></shdialogeditadviece>
  </div>
</template>
...
import shdialogmodal from "./SHDialogmodal.vue";
import shdialogaddabnormal from "./SHDialogAddAbnormal.vue";
import shdialogcompare from "./SHDialogCompare.vue";
import shdialogaddadviece from "./SHDialogAddAdvice.vue";
import shdialogeditadviece from "./SHDialogEditAdvice.vue";
...
export default {
  ...
  methods: {
    ...
    //辅助检查-图片弹出层
    showimg(path) {
      this.$refs.shdialogmodal.imgPath = path;
      this.$refs.shdialogmodal.dialogimgIsShow = true;
    },
    //返回父页面
    backParent() {
      this.$router.push({
        path: "/sh/" + this.parentName
      });
    ...
    via() {
      ...
      new Promise((resolve, reject) => {
        ...
        this.$axios
          .post(this.$api.BatchUpdateShFinalRstDetail, that.tableData2)
        ...
        this.isViaOrReject().then(res => {
          this.$axios
            .post(this.$api.PassSHFinalRst, {
              OrderCode: this.rowData.orderCode,
              OperatorCode: this.operatorCode
            })
            .then(res => {
              ...
              if (data.status == 1) {
                if (data.entity) {
                  ...
                  that.$message.error("审核通过失败，请重新提交！");
                }
              } else {
    ...
    isViaOrReject() {
      ...
      let pro = new Promise((resolve, reject) => {
        that.$axios
          .post(that.$api.IsCanRejectOrReviewSHUserByOrderCodeAndOperatorCode, {
            orderCode: that.orderCode,
            OperatorCode: that.operatorCode
          })
          .then(result => {
            if (result.status == 200 && result.data.status == 1) {
              resolve(result.data.entity);
            } else {
    ...
    reject() {
      this.isViaOrReject().then(res => {
        this.$refs.shdialogmodal.rejData = {
    ...
    getAdvice(orderCode, operatorCode) {
      ...
      that.loading1 = true;
      this.$axios
        .post(this.$api.GetShFinalRstDetailsByOrderCode, {
          orderCode,
          operatorCode
        })
        .then(function(response) {
          console.log(87, response.data.entity);
          that.loading1 = false;
          if (response.data.status == 1) {
            ...
            that.$message.error(
              `GetShFinalRstDetailsByOrderCode错误：${response.data.message}`
            );
          }
        })
        .catch(function(error) {
          that.$message.error(`GetShFinalRstDetailsByOrderCode错误：${error}`);
        });
    },
    //获取建议所属项目
    ...
    compare() {
      this.$refs.shdialogcompare.compareVisible = true;
    },
    //重新生成异常汇总
    ...
    reBuildAdvice(orderCode, operatorCode) {
      ...
      this.$confirm(
        ...
        .then(() => {
          this.$axios
            .post(this.$api.ZJGenerateMedicalGuidancesByOrderCode, {
              orderCode,
              operatorCode
            })
            .then(function(response) {
    ...
    reSort(orderCode) {
      var that = this;
      this.$axios
        .post(this.$api.SHGetAKeySequenceByOrderCode, {
          orderCode
        })
        .then(function(response) {
          if (response.data.status == 1) {
            ...
            that.$message.error(
              `SHGetAKeySequenceByOrderCode错误：${response.data.message}`
            );
          }
        })
        .catch(function(error) {
          that.$message.error(`SHGetAKeySequenceByOrderCode错误：${error}`);
        });
    },
    // 拖拽上移下移 弃用
    // drag() {
    ...
    deleteAdvice(innerCode) {
      ...
      this.$confirm(
        ...
        .then(() => {
          var that = this;
          this.$axios
            .post(this.$api.BatchDeleteShFinalRstDetail, innerCode)
            .then(function(response) {
              if (response.data.status == 1) {
                that.$message.success("删除成功");
    ...
    edit(row, index) {
      this.$refs.shdialogaddabnormal.flag = 2;
      this.$refs.shdialogaddabnormal.innerCode = row.innerCode;
      this.$refs.shdialogaddabnormal.itemCode = row.rptItemCodes;
      this.$refs.shdialogaddabnormal.addVisible = true;
    },
    editAdvise(row, index) {
      this.$refs.shdialogeditadviece.innerCode = row.innerCode;
      this.$refs.shdialogeditadviece.adviceType = row.adviceType;
      this.$refs.shdialogeditadviece.adviceEditVisible = true;
    },
    addAbnormalShow() {
      this.$refs.shdialogaddabnormal.flag = 1;
      this.$refs.shdialogaddabnormal.addVisible = true;
    },
    addAdvicelShow() {
      this.$refs.shdialogaddadviece.flag = 1;
      this.$refs.shdialogaddadviece.adviceVisible = true;
    },
