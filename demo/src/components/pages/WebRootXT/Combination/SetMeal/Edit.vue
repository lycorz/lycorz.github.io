<template>
  <div id="SetmealCreat" class="SetmealEdit">
    <el-dialog
      class="infoucs"
      width="660px"
      title="修改套餐"
...
export default {
  ...
  methods: {
    init() {
      ...
      if (this.Code != "") {
        that.$axios
          .get(that.$api.GetPackage, { params: { key: that.Code } })
          .then(res => {
            if (res.status == 200 && res.data.status == 1) {
              that.fromData = res.data.entity;
              that.fromData.dicPackageItemList.forEach(val => {
                that.transferValue.push(val.itemCode);
              });
            } else {
    ...
    submitForm() {
      this.$refs.createFrom.validate(valid => {
        if (valid) {
          ...
          //子项目赋值
          this.fromData.dicPackageItemList = new Array();
...
.SetmealEdit .el-select {
  width: 178px;
}
.SetmealEdit .el-textarea {
  width: 465px;
}
</style>
