<template>
  <div class="content SmstypeIndex">
    <div class="topTitle">
      <span>其他短信模版</span>
    </div>
      ...
      >
        ...
        <el-table-column
          ...
          align="left"
        ></el-table-column>
        <el-table-column property="templateContent" show-overflow-tooltip label="模板内容" align="left"></el-table-column>
    ...
    <SmstypeCreate ref="SmstypeCreate"></SmstypeCreate>
    <!-- <SmstypeEdit ref="SmstypeEdit"></SmstypeEdit> -->
  </div>
</template>

<script>
import SmstypeCreate from "./Create.vue";
export default {
  name: "SmstypeIndex",
  components: { SmstypeCreate },
  data() {
    return {
      name: "",
      total: 0,
  ...
  methods: {
    ...
    getData() {
      ...
      let that = this;
      this.$axios
        .get(this.$api.GetSmstypeList, {
          params: this.searchParams
        })
        .then(function(response) {
          if (response.data.status === 1) {
            that.tableData = response.data.entity.resDicSmstypes;
    ...
    /*  dialogfunction  */
    showCreateDialog() {
      let SmstypeCreate = this.$refs.SmstypeCreate;
      SmstypeCreate.isShow = true;
    },
    edit(index, row) {
      let SmstypeCreate = this.$refs.SmstypeCreate;
      SmstypeCreate.Code = row.typeCode;
      SmstypeCreate.isShow = true;
    },
    batchDelete() {
      let keys = this.multipleSelection.map(z => z.typeCode);
      if (keys.length <= 0) {
      ...
      this.$confirm(
        ...
        this.$axios
          .post(this.$api.BatchDeleteSmstype, keys)
          .then(res => {
